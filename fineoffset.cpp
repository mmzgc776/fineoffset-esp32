...
int DeviceReader::openPort(USB Usb)
{
...
int DeviceReader::find_device(USB Usb)
{
...
//Esta función lee el bloque fijo y retorna el bloque completo
uint8_t *DeviceReader::read_fixed_block(USB Usb, uint8_t *block)
{
    ...
    else
    {
        ...
        //delay(5000);
        //Valio madres, no se leeyo correctamente, restablecer la estación ?? yo no lo haría
        //return ERROR_ABORT;
    }
    //}
}
        ...
        if (result == 0)
        {
            ...
            for (int read = 0; read < 5; read++)
            {
                ...
                if (result == 0)
                {
                    ...
                    //A lo mejor sea más eficiente hacer memcpy
            ...
            if (!firstTime)
            {
                ...
                {
                    //Output nomas para verlos crudos
                    for (int i = 0; i < IN_BUFFER_SIZE; i++)
                    {
                        Serial.print(readBuffer[i], HEX);                        
                        Serial.print(" ");
...
int DeviceReader::getLastSentRecordfromFile()
{
    wh1080Work.lastRecord = -1;
    //Leer el archivo,
    //Abrir el archivo y escribir en la estructura de arriba la posicion de lastRecord
}

//Esta función lee el bloque 0 o bloque fijo y de ahí saca la posisción del buffer
...
int DeviceReader::readStationData(USB Usb, int currentPosition)
{
    Serial.println("DeviceReader/readStationData");
    ...
    //Serial.println(wh1080Work.lastRecord - (16 * numLectura));
    //Esta será la nueva posición, el ultimo registro más el numero de lectura * 16
    //readPosition = wh1080Work.lastRecord - (16 * numLectura);
    if (!read_block(Usb, currentPosition, &wh1080Work.recordBlock[0]))
    {
    ...
    if (wh1080Work.recordBlock[WH1080_STATUS] != 0x40)
    {
        // No!
    ...
    Serial.print("  Delay Raw:   ");
    Serial.print(wh1080Work.recordBlock[0]); //Ok
    Serial.print("  InTemp Raw:   ");
    Serial.print(wh1080Work.recordBlock[2]); //Ok
    Serial.print("  InHum Raw:   ");
    Serial.print(wh1080Work.recordBlock[1]); //Ok
    Serial.print("  Press Raw:   ");
    Serial.println(wh1080Work.recordBlock[7]); //ok
